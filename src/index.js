...
function checksExistsUserAccount(request, response, next) {
    ...
    if (!user) {
        return response.status(404).json({ error: "A conta não existe" })
    }
...
function checksCreateTodosUserAvailability(request, response, next) {
    ...
    if (!user.pro && user.todos.length + 1 > 10) {
        return response.status(403).json({ error: "a conta é free limite de 10 todos" });
    }
...
function checksTodoExists(request, response, next) {
    ...
    const userTodo = user.todos.find(acc => acc.id === id)
...
function findUserById(request, response, next) {
    ...
    if (!user) {
        return response.status(404).json({ error: "Id usuário não encontrado!!!" })
    }
